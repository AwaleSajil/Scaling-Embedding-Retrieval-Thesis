...
\usepackage[numbers]{natbib} % numerical citations (matches plainnat in numeric mode); enables \citeauthor, \citet, \citep
...
%The following loads tocbasic (via nomencl) before caption to avoid a koma-script 3.46+ hook ordering bug.
\usepackage[intoc]{nomencl}
\makenomenclature
\renewcommand{\nomname}{List of Symbols}
% Caption format per UAH manual: 10pt (footnotesize in 12pt doc), single-spaced,
% bold label ("Table 1.1"), period separator between label and title, centered.
\usepackage[font={footnotesize,singlespacing},labelfont=bf,labelsep=period,justification=centering,singlelinecheck=true]{caption}
\usepackage{tabularx} %To allow formatting Tables.
\usepackage{float} %For [H] placement specifier to pin floats in place.
...
%(nomencl is loaded earlier, before caption, to avoid a koma-script 3.46+ hook ordering issue)
