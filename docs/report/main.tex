...
%BASIC INFORMATION
\newcommand{\thesistitle}{Embedding Compression for Dense Retrieval: A Controlled Comparison of Post-hoc, Training-time, and Stacked Methods}
\newcommand{\studentname}{Sajil Awale}
\newcommand{\degree}{Master of Science}%e.g., Master of Science in Engineering, Doctor of Philosophy, etc.
\newcommand{\department}{Computer Science}% Do not include the words "The department of." Just write the name of your department.
\newcommand{\gradyear}{2026}% complete 4 digit year, e.g., 2022
...
\newcommand{\resadv}{Tathagata Mukherjee}
\newcommand{\comchair}{[Committee Chair Name]}
\newcommand{\reschair}{Tathagata Mukherjee} %If your research advisor and committee chair are the same person, enter his/her name on this line.
\newcommand{\commema}{Jacob Hauenstein}
\newcommand{\commemb}{Vaidyanath Areyur Shanthakumar}
...
\newcommand{\depchair}{Huaming Zhang}
...
\usepackage{multirow}
\usepackage{booktabs}
% Hyperlink colors: WCAG 2.1 AA requires that links be visually distinguishable from
% surrounding text. Internal cross-references and citations use a dark blue; external
% URLs use a slightly different blue so the reader can tell them apart at a glance.
\usepackage{xcolor}
\definecolor{linknavy}{RGB}{0,0,128}
\definecolor{urlblue}{RGB}{0,90,156}
\usepackage[colorlinks=true,allcolors=black]{hyperref} %Links stay clickable but render in black. It should be the last package before the document begins.

% List of Symbols is optional (per UAH manual) and omitted from this thesis.
...
%**********************************
%**********************************
%Rights and Responsibilities Page (Required)
%**********************************
%**********************************

% Per the UAH Graduate School manual, this page contains two sections:
%   Section 1 (required): generative AI disclosure statement.
%   Section 2 (optional): copyright notice (rendered only if \copyrightreg is defined
%   in the section above). This replaces the standalone Copyright Page from the
%   older template.
% The page has no heading and is intentionally NOT added to the Table of Contents.
\input{FrontMatter/RightsAndResponsibilities}
...
\include{Chapters/ch3_datasets}
\include{Chapters/ch4_methodology}
\include{Chapters/ch5_expirements_results}
...
\bibliographystyle{plainnat} %natbib-compatible version of `plain`; needed for \citeauthor/\citet/\citep to work.
