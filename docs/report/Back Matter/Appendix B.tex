...
\begin{table}[h]
    ...
    NanoBEIR subsets. Positive Cohen's $d$ favors the pre-trained
    model.}
...
\section{Binarization-Aware Training Exceeds Float32 on RoBERTa}
\label{sec:significance_bat_roberta}

Per-query MRR@10 is recorded for the RoBERTa float32 fine-tune and
for each of its two binarization-aware variants (BAT-STE and
...
in Table~\ref{tab:sig_bat_roberta}. Positive Cohen's $d$ favors the
binarized variant.
...
\begin{table}[h]
    ...
    \begin{tabular}{l r r p{0.45\textwidth}}
        ...
        Wins              & 155        & 167        & Queries where the binarized variant scores higher. \\
        ...
        Cohen's $d$       & $+0.117$   & $+0.207$   & Effect size on the difference vector; both favor the binarized variant. \\
...
binarized variant scores higher than the float32 fine-tune on
